  ...
  try {
    if (exportType === 'spotlights') {
      ...
      const { data, error } = await supabase
        .from('events')
        .select('slug,ts,event_name,utm_source,utm_campaign,country')
      ...
      const lines = (data || []).map(r =>
        row([r.slug, r.event_name, r.ts, r.utm_source, r.utm_campaign, r.country])
      );
    ...
    const { data, error } = await supabase
      .from('events')
      .select(EVENTS_COLUMNS.join(',').replace('event_name', 'event_name,name,device_hint'))
      .gte('ts', since)
      .order('ts', { ascending: false })
      .limit(10000);
    if (error) throw error;
    ...
    const lines = (data || []).map(r =>
      row([r.id, r.ts, r.event_name || r.name, r.slug, r.path, r.utm_source, r.utm_medium, r.utm_campaign, r.utm_content, r.utm_term, r.session_key, r.user_id, r.device_type || r.device_hint, r.os, r.browser, r.country, r.referrer])
    );
    const csv = [header, ...lines].join('\n');
